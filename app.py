...
st.markdown("##### ⚡ Powered by Gemini 2.5 Pro | v19.0 雙AI聯手優化版")
...
with st.sidebar:
    ...
    st.markdown("---")
    target_stock_sidebar = "2330" # Default
...
def calculate_breakout_probs(df, step_percent=1.0):
    ...
    # 使用指數衰減權重 (越近越重要)
    n = len(df)
    df['Weight'] = np.exp(np.linspace(-2, 0, n)) # 指數成長權重
...
def get_finmind_per(stock_id):
    url = "https://api.finmindtrade.com/api/v4/data"
    end_date = datetime.date.today()
    start_date = end_date - datetime.timedelta(days=14) # 抓兩週比較保險
    ...
    if data and "data" in data and data["data"]:
        latest = data["data"][-1]
        return {"P/E": latest.get("PER", 0), "Yield": latest.get("dividend_yield", 0)}
    return None
        ...
        if not valid_gemini: st.error("⛔ 請檢查 Gemini Key")
            ...
            with st.spinner(f"📡 戰情室連線中... 調閱 {target_stock} 全維度數據..."):
                ...
                if df is not None and not df.empty:
                    ...
                    with ai_col:
                        data_for_ai = compress_data_for_ai(df[['date', 'Close', 'MA60', '外資', '投信', 'K', 'D', 'MACD_Hist']])
                        ...
                        prompt_blue = f"你現在是 Alpha Strategist AI (v19.0 雙AI優化版)。任務：執行七大模組分析 {target_stock}。\n預載投資者輪廓：{investor_profile}\n【輸入情報】\n1. 技術籌碼：\n{data_for_ai}\n2. 基本面：{fundamentals}\n3. 營收：\n{rev_str}\n4. 宏觀：\n{news_str}\n請依照【基本面】、【技術籌碼】、【風險情境】、【戰略合成】章節撰寫。"
