...
def ask_gemini_combined_strategy(ticker, profile, wargame_on, red_style, data_context):
    ...
    if "Grok" in red_style:
        ...
        red_tone = "極度理性、科技視角、強調第一性原理。"
    else:
        red_persona = "華爾街空頭主力"
        red_tone = "冷血、無情、專找泡沫與估值過高點。"

    prompt = f"""
    你現在是 Alpha Strategist AI。請針對台股 {ticker} 進行一場深度的「兵棋推演」。
    (中間省略 Prompt 內容，保持原本的即可...)
    """
    
    # 🔥 關鍵修改：定義一個「候選名單」，包含你截圖裡的神秘型號
    candidate_models = [
        'models/gemini-1.5-flash',          # 標準版
        'models/gemini-1.5-pro',            # 標準 Pro
        'models/gemini-1.5-pro-latest',     # 強制最新
        'models/gemini-pro',                # 舊版保底 (通常最穩)
        'models/gemini-2.5-flash',          # 你的截圖有這個
        'models/gemini-2.5-flash-preview',  # 猜測名稱
        'models/gemini-3-pro-preview',      # 你的截圖有這個
    ]
    ...
    for model_name in candidate_models:
        try:
            ...
            # 如果成功，回傳結果並跳出 (可在這裡加註解告訴使用者用到哪個模型)
            return f"> 💡 AI 自動切換至模型：**{model_name}**\n\n" + response.text
    ...
    return f"❌ 所有模型都嘗試失敗。最後一次錯誤：{last_error}\n請確認您的 API Key 權限。"
